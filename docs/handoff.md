...
| `CRON_SECRET` | The bearer token Vercel Cron carries to `/api/cron/monthly-backup` and `/api/cron/calendar-sync` | **Both routes refuse everybody.** No monthly backup email, and the school's Google calendar stops being read. This is the safe direction — one route mails the whole database, and the other keeps publishing the last calendar it read |
| `GOOGLE_CALENDAR_ID` | The school's own Google calendar — `pharosacademy.net@gmail.com`. A whole iCal URL works too, for a calendar that is not public. **Not a credential:** the school's calendar is public, and this is only its name (#153) | The nightly sync 500s and writes nothing. The calendar page keeps the events it already has, and anything the school adds in Google stops arriving |
