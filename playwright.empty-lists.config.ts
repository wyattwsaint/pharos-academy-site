 ...
 * empty and their empty states would ship untested. Announcements gained a
 * delete in #258 and the rest follow, but emptying a list through the admin is
 * a republish per row and one screen's fixture would be another's missing
 * seed — so the lists are still emptied here, at the database, rather than by
 * driving the button. `E2E_EMPTY_LISTS` makes the suite server delete
