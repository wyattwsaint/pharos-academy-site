...
function publicRoutes(classSlugs: readonly string[]): PublicRoute[] {
  return [
    ...
    /*
     * One route per class, generated from a slug list rather than listed.
     *
     ...
     * it is a build-time constant and the build knows nothing else. Every surface
     * that runs against the database — the sitemap, `llms.txt` and whole-site
     * republishing — passes the **live** slugs instead (`publicRoutesFor`, and
     * `livePublicRoutes` above it). A course Jill adds in the editor is a row,
     * not a seed entry, so a list built from the seed would leave her new class
     * unrevalidated and out of the sitemap while the admin told her it was live.
     */
    ...classSlugs.map((slug) => ({
      path: classPath(slug),
      priority: 0.7,
 ...
 * destination check, and the database-down suite. Anything that renders against
 * the database wants `publicRoutesFor` with the live slugs instead.
 */
export const PUBLIC_ROUTES: readonly PublicRoute[] = publicRoutes(
  CATALOGUE.map((course) => course.slug),
);

/**
 * The same list, with one class route per slug the school actually offers now.
 *
 * The slugs come from the store, so a class added, renamed or removed in the
 * course editor reaches the sitemap, `llms.txt` and whole-site republishing in
 * the same save that wrote it (#24).
 */
export function publicRoutesFor(classSlugs: readonly string[]): PublicRoute[] {
  return publicRoutes(classSlugs);
}

/** Every public path, in list order. */
export function publicPaths(routes: readonly PublicRoute[] = PUBLIC_ROUTES): string[] {
 ...
 */
export function revalidatablePaths(routes: readonly PublicRoute[] = PUBLIC_ROUTES): string[] {
  return routes.filter((route) => route.delivery !== 'on-request').map((route) => route.path);
}
